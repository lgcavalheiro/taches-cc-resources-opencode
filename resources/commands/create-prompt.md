...
→ **IMMEDIATELY use ask the user a structured question with options (use the question tool or present options inline)** with:
...
Generate 2-4 questions using ask the user a structured question with options (use the question tool or present options inline) based ONLY on genuine gaps.
...
<decision_gate>
After receiving answers, present decision gate using ask the user a structured question with options (use the question tool or present options inline):
...
<action>
If user chooses #1, invoke via execute the command `/run-prompt 005`
</action>
</single_prompt_scenario>
...
<actions>
If user chooses #1, invoke via execute the command `/run-prompt 005 006 007 --parallel`
If user chooses #2, invoke via execute the command `/run-prompt 005 006 007 --sequential`
</actions>
</parallel_scenario>
...
<actions>
If user chooses #1, invoke via execute the command `/run-prompt 005 006 007 --sequential`
If user chooses #2, invoke via execute the command `/run-prompt 005`
</actions>
</sequential_scenario>
...
<success_criteria>
- Intake gate completed (ask the user a structured question with options (use the question tool or present options inline) used for clarification if needed)
...
- User choice executed (execute the corresponding command invoked if user selects run option)
</success_criteria>

<meta_instructions>

- **Intake first**: Complete step_0_intake_gate before generating. Use ask the user a structured question with options (use the question tool or present options inline) for structured clarification.
...
- After saving, present the decision tree as inline text (not ask the user a structured question with options (use the question tool or present options inline))
- Use the execute the corresponding slash command to invoke /run-prompt when user makes their choice
</meta_instructions>
