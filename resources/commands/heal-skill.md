...
Analyze the conversation to detect which skill is running, reflect on what went wrong, propose specific fixes, get user approval, then apply changes. Do NOT commit changes. Only commit if the user explicitly requests it.
</objective>

<context>
Skill detection: Run the command `ls -1 ./skills/*/SKILL.md | head -5` to get this information.
</context>
...
5. **Apply** changes (do NOT commit unless user explicitly requests)
</workflow>
</quick_start>
...
2. Let me review first before applying
...
3. Confirm completion with file list

Do NOT commit changes. Only commit if the user explicitly requests it.
</step_6>
</process>
