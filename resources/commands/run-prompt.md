...
---
<context>
Git status: Run the command `git status --short` to get this information.
Recent prompts: Run the command `ls -t ./prompts/*.md | head -5` to get this information.
</context>
...
2. Delegate as sub-task using subagent/task tool with subagent_type="general-purpose"
3. Wait for completion
4. Archive prompt to `./prompts/completed/` with metadata
5. Do NOT commit changes. Only commit if the user explicitly requests it.
6. Return results
   </single_prompt>
...
2. **Spawn all subagent/task tools in a SINGLE MESSAGE** (this is critical for parallel execution):
   <example>
   Use subagent/task tool for prompt 005
   Use subagent/task tool for prompt 006
   Use subagent/task tool for prompt 007
...
2. Spawn subagent/task tool for first prompt
3. Wait for completion
4. Archive first prompt
5. Read second prompt file
6. Spawn subagent/task tool for second prompt
7. Wait for completion
8. Archive second prompt
9. Repeat for remaining prompts
10. Archive all prompts with metadata
11. Do NOT commit changes. Only commit if the user explicitly requests it.
...
- For parallel execution: ALL subagent/task tool calls MUST be in a single message
