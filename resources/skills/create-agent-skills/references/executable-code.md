<when_to_use_scripts>
Even if Opencode could write a script, pre-made scripts offer advantages:
...
Make clear whether Opencode should:
...
<how_scripts_work>
When Opencode executes a script via bash:
1. Script code never enters context window
2. Only script output consumes tokens
3. Far more efficient than having Opencode generate equivalent code
</how_scripts_work>
</when_to_use_scripts>
...
<solve_dont_punt>
Handle error conditions rather than punting to Opencode.
...
```python
def process_file(path):
    # Just fail and let Opencode figure it out
...
- **opencode.ai**: Can install packages from npm and PyPI
- **Opencode API**: No network access and no runtime package installation
</runtime_constraints>
...
Without the server prefix, Opencode may fail to locate the tool, especially when multiple MCP servers are available.
</mcp_tool_references>
