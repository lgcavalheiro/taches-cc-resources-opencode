...
- No XML tags
- No reserved words: "opencode"opencode
...
- Reserved words: `opencode-helper`, `opencode-tools`
...
When user asks about revenue, Opencode reads only finance.md. Other files stay on filesystem consuming zero tokens.
</pattern>
...
<critical_rules>
**Keep references one level deep**: All reference files should link directly from SKILL.md. Avoid nested references (SKILL.md → advanced.md → details.md) as Opencode may only partially read deeply nested files.
...
Opencode navigates your skill directory using bash commands:
...
Keep references one level deep from SKILL.md. Opencode may only partially read nested files (SKILL.md → advanced.md → details.md).
</pitfall>
