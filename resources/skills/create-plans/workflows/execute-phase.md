...
Aggregate results → SUMMARY → report back
```
...
This is an autonomous plan (no checkpoints). Execute all tasks, create SUMMARY.md in phase directory.

Follow all deviation rules and authentication gate protocols from the plan.

When complete, report: plan name, tasks completed, SUMMARY path."
```
   ...
   B. If routing = Subagent:
      ...
      **Your responsibilities:**
      ...
      - DO NOT create SUMMARY.md (will be created after all segments complete)
       - DO NOT commit (only commit when the user explicitly requests it)
...
**Do NOT commit automatically.** Only commit when the user explicitly requests it. You may suggest a commit, but never execute one without explicit user approval.
</step>
