...
All prompting best practices apply. Be clear, be direct, use XML structure. Assume Opencode is smart - only add context Opencode doesn't have.

### 2. SKILL.md Is Always Loaded

When a skill is invoked, Opencode reads SKILL.md. Use this guarantee:
...
- **workflows/** - Multi-step procedures Opencode follows
- **references/** - Domain knowledge Opencode reads for context
- **templates/** - Consistent output structures Opencode copies and fills (plans, specs, configs)
- **scripts/** - Executable code Opencode runs as-is (deploy, setup, API calls)
...
templates/:
  Output structures Opencode copies and fills
  (plans, specs, configs, documents)

scripts/:
  Executable code Opencode runs as-is
