...
### Suggest commit after major changes
```json
{
  "hooks": {
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/path/to/suggest-commit.sh"
          }
        ]
      }
    ]
...
if ! git diff --quiet; then
  echo '{"systemMessage": "You have uncommitted changes. Consider asking the user if they would like to commit."}'
fi
```

**Note:** Auto-committing is NOT recommended. Always let the user decide when to commit.
