...
Select the most important task from the implementation plan, implement it correctly, validate it works. Do NOT commit. Only commit if explicitly requested by the user.
...
4. Validate
   ...
   - Do NOT commit. Only commit when explicitly requested by the user.
...
- Changes NOT auto-committed (only commit when explicitly requested)
